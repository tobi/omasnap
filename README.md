...
- Clip-out: in Select, `V` cycles clip shape (rectangle, ellipse, lasso) and a
  ...
  on: drag a box around the object (leave a little background in the
  corners) then Snap reads what is connected under the pointer inside that
  box. A scan-dot traces the outline (OCR-style) then the mask locks. Rect
  ...
  hole infill (transparent by default; match-surroundings swatch; `T` /
  `1`–`8` / Sample from image `I` / `#` while the mask is locked). Drag inside to lift a layer with alpha outside the path.
